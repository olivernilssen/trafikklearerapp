const imgSource = {
    Veikryss: {
        Høyrekryss: {
            X: require('../../assets/intersections/hoyrekryss/veikryss-hoyre-X.png'),
            'X (gangfelt)': require('../../assets/intersections/hoyrekryss/veikryss-hoyre-X-gangfelt.png'),
            T: require('../../assets/intersections/hoyrekryss/veikryss-hoyre-T.png'),
            'T (gangfelt)': require('../../assets/intersections/hoyrekryss/veikryss-hoyre-T-gangfelt.png'),
            Y: require('../../assets/intersections/hoyrekryss/veikryss-hoyre-Y.png'),
            'Y (gangfelt)': require('../../assets/intersections/hoyrekryss/veikryss-hoyre-Y-gangfelt.png'),
        },
        Forkjørskryss: {
            X: require('../../assets/intersections/forkjorskryss/veikryss-forkjors-X.png'),
            'X (gangfelt)': require('../../assets/intersections/forkjorskryss/veikryss-forkjors-X-gangfelt.png'),
            T: require('../../assets/intersections/forkjorskryss/veikryss-forkjors-T.png'),
            'T (gangfelt)': require('../../assets/intersections/forkjorskryss/veikryss-forkjors-T-gangfelt.png'),
            Y: require('../../assets/intersections/forkjorskryss/veikryss-forkjors-Y.png'),
            'Y (gangfelt)': require('../../assets/intersections/forkjorskryss/veikryss-forkjors-Y-gangfelt.png'),
        },
        Lyskryss: {
            X: require('../../assets/intersections/lyskryss/veikryss-lys-X.png'),
            'X (gangfelt)': require('../../assets/intersections/lyskryss/veikryss-lys-X-gangfelt.png'),
            // 'X-pil': require('../../assets/intersections/lyskryss/veikryss-lys-X-pil.png'),
            T: require('../../assets/intersections/lyskryss/veikryss-lys-T.png'),
            'T (gangfelt)': require('../../assets/intersections/lyskryss/veikryss-lys-T-gangfelt.png'),
            // 'T-pil': require('../../assets/intersections/lyskryss/veikryss-lys-T-pil.png'),
            Y: require('../../assets/intersections/lyskryss/veikryss-lys-Y.png'),
            'Y (gangfelt)': require('../../assets/intersections/lyskryss/veikryss-lys-Y-gangfelt.png'),
            // 'Y-pil': require('../../assets/intersections/lyskryss/veikryss-lys-Y-pil.png'),
        },
    },
    Rundkjoring: {
        'Ettfelts rundkjøring': {
            'Ett felt': require('../../assets/roundabouts/1feltinn1feltut.png'),
            'Ett felt med gangfelt': require('../../assets/roundabouts/1feltinn1feltut-gangfelt.png'),
        },
        'Tofelts rundkjøring': {
            'To felt': require('../../assets/roundabouts/2feltinn1feltut.png'),
            'To felt med gangfelt': require('../../assets/roundabouts/2feltinn1feltut-gangfelt.png'),
        },
        'Firefelts rundkjøring': {
            'Fire felt': require('../../assets/roundabouts/2feltinn2feltut.png'),
            'Fire felt med gangfelt': require('../../assets/roundabouts/2feltinn2feltut-gangfelt.png'),
        },
    },
};

export default imgSource;
